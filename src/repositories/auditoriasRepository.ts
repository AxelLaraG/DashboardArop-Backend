...
class AuditoriasRepository {
  async getAuditorias(): Promise<RowDataPacket[]> {
  ...
  async setNewAuditoria(auditoria: Partial<Auditorias>): Promise<number> {
    const query = `INSERT INTO AUDIROTIAS (ID_USUARIO, TABLA, TRANSACCION, FECHA, USER_AGENT) VALUES (?,?,?,NOW(),?)`;
  ...
  async getAuditoriasByTable(table: string): Promise<RowDataPacket[]> {
    const query = "SELECT * FROM AUDITORIAS WHERE TABLE = ?";
